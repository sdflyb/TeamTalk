...
{
	uint64_t cur_time = get_tick_count(); // 获取当前时间，单位毫秒
...
{
	if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) // 连接的是 client
	{
		if (curr_tick > m_last_recv_tick + CLIENT_TIMEOUT) // 超时关闭
		{
			Close();
		}
	}
	else // 连接的是 msg_server
	{
		if (curr_tick > m_last_send_tick + SERVER_HEARTBEAT_INTERVAL) // 每 5 秒发送一个心跳包
		{
		...
		if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT)  // 超时关闭
		{
