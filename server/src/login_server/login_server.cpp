...
int main(int argc, char *argv[])
{
	...
	signal(SIGPIPE, SIG_IGN); // 处理 SIGPIPE 信号

	CConfigFileReader config_file("loginserver.conf"); // 读取配置文件
	...
	uint16_t client_port = atoi(str_client_port); // client 端口
	uint16_t msg_server_port = atoi(str_msg_server_port); // msg_server 端口
	uint16_t http_port = atoi(str_http_port); // http 端口
	strMsfsUrl = str_msfs_url;
	strDiscovery = str_discovery;

	pIpParser = new IpParser();

	int ret = netlib_init(); // linux 下什么都没发生
...
//***********************************************************************************************************
	...
	init_login_conn(); // 注册一个定时器
	init_http_conn();  // 注册一个定时器
