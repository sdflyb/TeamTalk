...
{
  public:
	CStrExplode(char *str, char seperator); // 将 str 用 seperator 分割开
	virtual ~CStrExplode();

	uint32_t GetItemCnt() { return m_item_cnt; }			 // 解析出来的个数
	char *GetItem(uint32_t idx) { return m_item_list[idx]; } // 返回解析的第 indx 快
  private:
	uint32_t m_item_cnt;
	char **m_item_list;
};
